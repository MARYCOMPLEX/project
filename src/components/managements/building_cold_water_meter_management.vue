<template>
  ...
  export default {
    ...
    methods: {
      fetchData() {
        this.loading = true;
        axios.post('http://127.0.0.1:8000/smartschools/Get_Dormitory_Cool_Water_meter/')
          .then(response => {
      ...
      submitForm() {
        axios.post('http://127.0.0.1:8000/smartschools/Add_Dormitory_Cool_Water_meter/', this.formData)
          .then(response => {
      ...
      deleteEntry(row) {
        axios.post('http://127.0.0.1:8000/smartschools/Decrease_Dormitory_Cool_Water_meter/', { 'Id': row.Id })
          .then(response => {
