<template>
  ...
  export default {
    ...
    methods: {
      ...
      submitForm() {
        axios.post('http://127.0.0.1:8000/smartschools/Add_Dormitory_Hot_water/', this.formData)
          .then(response => {
      ...
      deleteEntry(row) {
        axios.post('http://127.0.0.1:8000/smartschools/Decrease_Dormitory_Hot_water/', { 'Id': row.Id })
          .then(response => {
