<template>
  ...
  export default {
    ...
    methods: {
      ...
      submitForm() {
        axios.post('http://127.0.0.1:8000/smartschools/Add_Building_ammeter/', this.formData)
          .then(response => {
      ...
      deleteEntry(row) {
        axios.post('http://127.0.0.1:8000/smartschools/Decrease_Building_ammeter/', { 'Id': row.Id })
          .then(response => {
